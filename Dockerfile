...
RUN if [ "${BASE_IMAGE}" = "cgr.dev/chainguard/wolfi-base" ]; then \
  apk add --no-cache gcc=14.2.0-r13 go make git; \
  fi
...
RUN apk add --no-cache git python-3.13 py3.13-pip bash tini procps libreoffice docker perl-utils sqlite sqlite-dev curl kubectl jq
